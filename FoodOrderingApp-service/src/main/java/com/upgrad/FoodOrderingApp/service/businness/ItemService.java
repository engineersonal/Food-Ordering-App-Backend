...
public class ItemService {
    ...
    */
    public List<ItemEntity> getItemsByPopularity(RestaurantEntity restaurantEntity) {
       ...
       //Calls sortMapByValues method of utilityProvider and get sorted map by value.
       Map<String,Integer> sortedItemCountMap = utilityProvider.sortMapByValues(itemCountMap);

       //Creating the top 5 ItemEntity list
        List<ItemEntity> sortedItemEntities = new LinkedList<>();
        Integer count = 0;
        for(Map.Entry<String,Integer> item:sortedItemCountMap.entrySet()){
            if(count < 5) {
                //Calls getItemByUUID to get the ItemEntity
                sortedItemEntities.add(itemDao.getItemByUUID(item.getKey()));
                count = count+1;
            }else{
                break;
            }
